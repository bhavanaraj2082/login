...
if (!selectedOption ) {
errors.selectedOption = 'Please select one of the reference options';
...
if (confirmationNumber && !/^[A-Za-z0-9-]+$/.test(confirmationNumber)) {
errors.confirmationNumber = 'Confirmation number should contain only alphanumeric characters and dashes';
} else {
...
errors.itemNumber = 'Item number should contain only alphanumeric characters and dashes';
} else {
...
<div class="lg:w-1/2 w-full pb-6 mx-auto h-full">
  <h2 class="text-primary-400 font-semibold text-base pb-6">Invoice</h2>
  <input hidden name="issueName" value="Invoice" />
  <div class="mb-4">
  <!-- svelte-ignore a11y-label-has-associated-control -->
  <label class="block mb-2 text-sm">
    Please provide one of the following
  </label>
  <div class="flex flex-col md:flex-row lg:flex-row justify-between w-3/4 p-2">
    ...
    <input
      ...
      on:input={() => { validateField('selectedOption'); clearError('selectedOption'); }}
    ...
    <input
      ...
      class="form-radio w-3.5 h-3.5 text-primary-500 focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-primary-500"
    />
    <label for="invoiceNumber" class="ml-2">Invoice Number</label>
    </div>
  
  
  </div>
  {#if errors.selectedOption}
  <p class="text-red-500 text-xs">{errors.selectedOption}</p>
  {/if}
  
  <input
    name="selectOptionNumber"
    type="text"
    class="border border-gray-300 shadow-sm focus:outline-none focus:ring-1 focus:ring-primary-400 focus:border-primary-400 p-2 w-full lg:w-3/4 md:w-3/4 text-sm rounded-md"
    bind:value={selectOptionNumber}
    on:input={() => validateField('selectOptionNumber')}
    
  />
  {#if errors.selectOptionNumber}
    <p class="text-red-500 text-xs mt-1">{errors.selectOptionNumber}</p>
  {/if}
  </div>
  ...
  <input
    ...
    on:input={() => validateField('confirmationNumber')}
  ...
  <p class="text-red-500 text-xs mt-1">{errors.confirmationNumber}</p>
  {/if}
  <input
    name="itemNumber"
    type="text"
  ...
  <p class="text-red-500 text-xs mt-1">{errors.itemNumber}</p>
  {/if}
  </div>
</div>
<div class="lg:w-1/2 w-full pb-6 mx-auto h-full">
