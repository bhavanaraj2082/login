...
let companyName =  data?.profile?.companyName|| "";
  ...
  if (!fieldName || fieldName === 'resetemail') {
	  if (!resetemail || !/^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}$/.test(resetemail)) {
		errors.resetemail = 'Please enter a valid email address';
	  } else {
		delete errors.resetemail;
	  }
	}
  ...
  if (!assistance || 
    ...
    errors.assistance = 'Message is required and must not contain HTML tags or scripts.';
  } else {
...
>
    <div class=" w-full pb-6 h-full">
      ...
      on:input={() => validateField('resetemail')}
    ...
    {/if}
      <div class="mt-4">
        ...
        <textarea rows="5" name="assistance"
         ...
         class="w-full border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-1 focus:ring-primary-400 focus:border-primary-400 p-2 text-sm mt-2" required></textarea>
