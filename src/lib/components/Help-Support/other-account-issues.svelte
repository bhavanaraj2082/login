...
<div class="w-full p-4">
    ...
    <div class=" flex flex-col lg:flex-row justify-between h-full">
      <div class="lg:w-1/2 w-full pb-6 mx-auto h-full">
        ...
        <div>
          ...
          ></textarea>
          {#if errors.assistanceMessage}
          <p class="text-red-500 text-xs mt-1">{errors.assistanceMessage}</p>
          {/if}
        </div>
      </div>
      <div class="lg:w-1/2 w-full pb-6 mx-auto h-full">
  ...
  {#if message != ""}
    <h2
      class="text-center text-green-500 font-semibold text-base w-3/4"
    >
      {message}
    </h2>
  {:else if errormessage != ""}
    <h2
      class="text-center text-red-500 font-semibold text-base w-3/4"
    >
      {errormessage}
    </h2>
  {/if}
