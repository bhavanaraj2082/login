...
errors.assistance = 'Description is required and must not contain HTML tags or scripts.';
} else {
...
<p class="text-base pb-6 hidden">
  Are you located in the United States? If so, you can initiate a return
  from your Order History page!<a href="/order-history"
    >Click here to get started</a
  >
</p>
<p class="text-sm pb-6 hidden">
  Not located in the United States? Don't worry, we're working on rolling
  out this feature globally soon! Until then, you can still initiate a
  ...
  <div class="mt-2">
    <label class="block text-sm py-2">
      <input
        ...
        bind:group={issue}
      class="form-radio w-3.5 h-3.5 text-primary-500 focus:outline-none mr-2 focus:ring-2 focus:ring-offset-1 focus:ring-primary-500"
        
        on:input={() => { validateField('issue'); clearError('issue'); }}
      />
...
<textarea rows="5" name="assistance" bind:value={assistance} class="w-full border p-2 text-sm  border-gray-300 focus:outline-none focus:ring-0 focus:ring-primary-300 focus:border-primary-300" 
