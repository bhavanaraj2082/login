...
<div class="p-6 max-w-7xl mx-auto w-11/12">
    ...
    <div class="flex flex-wrap justify-between items-center mb-8">
        ...
        <div class="flex w-full md:w-auto md:space-x-4 text-primary-400 font-semibold">
          <button
            on:click={addAllToCart}
            class="hover:underline w-full md:w-auto">
            Add all to cart
          </button>
          <button
            on:click={clearAll}
            class="hover:underline w-full md:w-auto ml-auto md:ml-0">
            Clear all
          </button>
        </div>
      </div>
    ...
    <div class="space-y-6">
        {#each favouriteItems as item, index}
            <div class="flex flex-wrap items-start p-6 border border-primary-200 rounded-lg">
                <img src={item.image} alt="img" class="w-24 h-24 object-cover rounded-lg" />

                <div class="ml-6 md:ml-2 flex-1 space-y-2">
                ...
                <div class="flex flex-col items-start sm:items-end w-full sm:w-auto mt-4 sm:mt-0 space-y-4">
                    <div class="flex justify-between w-full sm:w-auto items-center space-x-6 text-primary-400 font-semibold">
                        ...
                        <button on:click={() => removeItem(index)} class="hover:underline flex items-center space-x-1">
                            <span class="text-sm sm:text-base">remove</span>
                    ...
                    <div class="flex flex-wrap sm:flex-nowrap justify-between items-center w-full space-x-0 sm:space-x-4 mt-2 ">
                        <span class="font-semibold text-gray-800 text-lg">{item.price}</span>
                        <div class="flex items-center border rounded-lg px-2 py-1 space-x-2 sm:px-3 sm:py-2">
