...
<div>
	...
	<div class="flex items-center">
		<button class=" p-2" on:click={prev} disabled={currentIndex === 0}>
			<Icon icon="lsicon:double-arrow-left-outline" class=" text-3xl  text-primary-500" />
		</button>
			...
			<div class="grid grid-cols-4 gap-4">
				{#each data.slice(currentIndex, currentIndex + 4) as product}
					<div class="bg-white shadow-lg rounded p-4">
						<div class="flex">
							<img src={product.imageSrc} alt={product.productName} class="w-1/3 rounded-lg" />
							<div class="ml-4">
								<h3 class="text-xs">{product.manufacturer}</h3>
		...
		<button class="p-2" on:click={next} disabled={currentIndex + getCardsToShow() >= data.length}>
			<Icon icon="lsicon:double-arrow-right-outline" class="text-3xl  text-primary-500" />
		</button>
	</div>
</div>
	...
	>
		...
		>
			...
			>
				...
				<div class="border-b-1 border-primary-500 flex justify-between items-center m-4">
				...
				<div class="flex justify-between px-4 items-start text-sm">
					...
					<span>
						{previewProduct.priceSize[0].price} / {previewProduct.priceSize[0].size}
					</span>
				</div>
				<div class="flex items-center mt-4 justify-evenly flex-col lg:flex-row p-4 w-full">
					<div class="flex flex-row">
						...
						>
							...
							<input
								...
								on:input={(e) => (quantityInput = Math.max(1, parseInt(e.target.value) || 1))}
							/>
							<!-- svelte-ignore a11y-click-events-have-key-events -->
							<!-- svelte-ignore a11y-no-static-element-interactions -->
							<span
						...
						<input
							...
							hidden
						/>
						<input type="text" bind:value={quantityInput} name="quantity" hidden />
					</div>
					{#if message}
						<span class="text-sm ml-1">{message}</span>
					{/if}
					<button class="text-primary-500 font-semibold text-sm mt-2">CHECK AVAILABILITY</button>
				</div>
			</div>
		</div>
	</form>
