<script>
	...
	function addToCart(product) {
		...
		const cart = getCart();
		const existingProduct = cart.find((item) => item.productNumber === product.partNumber);
...
{#if showModal}
	<div class="fixed inset-0 bg-gray-800 bg-opacity-50 flex justify-center items-center z-50">
		<div class="bg-white p-6 rounded-lg w-full sm:w-3/4 md:w-2/3 lg:w-1/2 xl:w-5/12 relative">
			{#if showCartMessage}
				<div class=" bg-green-400 text-white my-2 text-center">Item added to cart!</div>
			{/if}
				...
				{#if selectedPrice}
					<div class="mt-5 flex gap-6 items-center justify-between sm:justify-start">
						<p class="text-base sm:text-lg ml-2">
							Price: <span class="font-semibold text-2xl">{selectedPrice.price}</span>
						</p>
					</div>
				{/if}
				<div class="mt-4">
