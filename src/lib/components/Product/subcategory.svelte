...
<div>
    <div class="max-w-7xl mx-auto py-6 px-6 lg:px-8">
        <div class="pl-4">
            <h2 class="text-3xl font-bold text-primary-500">Our SubCategory</h2>
        ...
        <div class="mt-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
             {#each subcategories as subcategory}
                <div class="bg-white shadow-lg rounded-sm overflow-hidden">
                    ...
                    <div class="p-4">
                        <a href="/products/{subcategory.expand.category.urlName}/{subcategory.urlName}">
                            <h3 class="text-lg font-semibold text-heading hover:underline">{subcategory.name}</h3>
                        </a>
                    </div>
                </div>
            {/each} 
