<script>
  ...
  $: filteredCategories = originalCategories.filter(
    (category) =>
...
<div class="w-full mb-10 lg:px-4 md:w-11/12 mx-auto max-w-7xl">
  <div class="md:pl-0">
    <h1
      class="sm:hidden block px-6 sm:px-0 mb-2 sm:mb-0 font-bold sm:text-2xl text-lg text-heading"
    >
      Our Category
    </h1>
    <div
    ...
    >
      <h1
        class="sm:block hidden px-6 sm:px-0 mb-2 sm:mb-0 font-bold sm:text-2xl text-lg text-heading"
      >
        Our Category
      </h1>
      <div class="relative">
  ...
  {#if filteredCategories.length > 0}
    ...
    >
      {#each filteredCategories as category}
        <div
          class="relative group bg-white shadow-lg rounded-lg overflow-hidden transition-shadow duration-300 hover:shadow-xl"
        >
          <div
