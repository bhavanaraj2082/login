<script>
  ...
  function isValidName() {
    ...
    const nameIsValid =
      namePattern.test(name) && name.length >= 3 && name.length <= 50;
    return nameIsValid;
  }
  ...
  const handleSubmit = async (event) => {
    ...
    if (feedback.length === 0) {
      ...
      errors.message =
        "Feedback must include at least one alphabetic character.";
      showFeedbackError = true;
      formValid = false;
    } else {
  ...
  >
    ...
    >
      ...
      <p class="md:text-md text-xs text-gray-700 mt-4 mb-6">
        Thank you for submitting your Feedback! We have received it and we will
        look into that.
      </p>
  ...
  >
    ...
    >
      <p class="sm:text-md text-sm text-gray-700 mb-6">
        There was an issue with submitting the form. Please try again after a
        while.
      </p>
...
{:else}
  <h1
    class="font-bold text-lg md:text-2xl pl-1 pb-5 w-11/12 max-w-7xl justify-center mx-auto"
  >
    Feedback
  </h1>
  <section
    class="w-11/12 max-w-7xl flex flex-wrap justify-center mx-auto font-roboto bg-white mb-10"
  >
    <div class="w-full border border-gray-300 rounded-lg p-2">
      <form
        ...
        id="feedback-form"
        class="p-4"
        use:enhance={({ event, formData: data }) => {
          if (data) {
        ...
        <div class="space-y-12 font-workSans">
          <div class="">
            <h2 class="text-sm font-semibold text-gray-900 mb-4">
              Chemikart feedback<span class="text-red-500 text-xs font-semibold"
                >*</span
              >
            </h2>
            <p class="mt-1 mb-4 sm:text-sm text-xs text-gray-600">
              If you come across any issues on our website, let us know! We’ll
              work on fixing them right away.
            </p>
            ...
            <fieldset class="border-b border-gray-900/10 pb-12">
              <div class="mt-2 space-y-4 text-xs">
                ...
                {#if selectedRating === "Website design & usability"}
                  <legend class="text-sm font-semibold text-gray-900"
                    >Enhancing Website Performance and Customer Satisfaction</legend
                  >
                  {#each ["The website is loading slowly", "Poor Mobile Responsiveness", "Poor Search Functionality", "Lack of Payment Options", "Good Website design"] as issue}
                    <div class="flex items-center gap-2">
                      ...
                      <label
                        for={issue}
                        class="font-base text-gray-900 cursor-pointer"
                        >{issue}</label
                      >
                    </div>
                  {/each}
              ...
              {#if showIssueError}
                <div class="text-red-500 sm:text-xs text-2s font-medium mt-3">
                  *Required
                </div>
              {/if}
            </fieldset>
            ...
            >
              <div class="sm:col-span-3">
                <legend class="text-sm font-semibold text-gray-900 pt-2 pb-3"
                  >Your search matters!
                </legend>
                <label
                  for="requirement"
                  class="block sm:text-sm text-xs font-base text-gray-900 pb-1"
                  >"Did you find what you needed on our website?"</label
                >
                <select
                  id="requirement"
                  name="requirement"
              ...
              <div class="sm:col-span-2 sm:col-start-1">
                <label
                  for="name"
                  class="block sm:text-sm text-xs font-base text-gray-900 pb-1"
                  >User Name<span class="text-red-500 text-xs font-semibold"
                    >*</span
                  ></label
                >
                <input
                ...
                {:else if (formData.name.length > 0 && formData.name.trim().length < 3) || (formData.name.length > 0 && formData.name.trim().length > 50)}
                  <span class="text-red-500 text-xs font-normal"
                    >User Name must be between 3 and 50 characters</span
                  >
                {/if}

                {#if showNameError}
                  <div class="text-red-500 sm:text-xs text-2s font-medium mt-1">
                    *Required
                  </div>
                {/if}
              </div>
      ...
      /> -->
                <label
                  for="email"
                  class="block tsm:text-sm text-xs font-base text-gray-900 pb-1"
                  >Email<span class="text-red-500 text-xs font-semibold">*</span
                  ></label
                >
                <form
                  action="?/verifyemail"
         ...
         /> -->
              ...
              <div class="sm:col-span-2">
                <label
                  for="url"
                  class="block sm:text-sm text-xs font-base text-gray-900 pb-1"
                  >URL/<span class="text-red-500 text-xs font-semibold">*</span
                  ></label
                >
                <input
                  type="url"
                ...
                {#if showUrlError}
                  <div class="text-red-500 sm:text-xs font-medium mt-1">
                    *Required
                  </div>
                {/if}
              ...
              <div class="sm:col-span-4">
                <label
                  for="feedback"
                  class="block sm:text-sm text-xs font-base text-gray-900 pb-1"
                  >Tell us what we can do better!<span
                ...
                <textarea
                  ...
                  on:input={() => {
                    ...
                    if (normalizedMessage.length === 0) {
                      ...
                      normalizedMessage.length > 700
                    ) {
                      errors.message =
                        "Feedback must be between 5 and 700 characters.";
                    } else if (/<[^>]*>/.test(normalizedMessage)) {
                      errors.message =
                        "HTML tags are not allowed in the message.";
                    } else if (!/[A-Za-z]/.test(normalizedMessage)) {
                      errors.message =
                        "Feedback must include at least one alphabetic character.";
                    } else {
                      delete errors.message;
