<script>
	...
	$: suggestions = $searchResult.filter((component) =>
...
<nav class="my-3 justify-between relative w-full mx-auto">
	...
	<div class="mx-auto flex items-center justify-between max-md:py-0 py-4 px-4">
		...
		<div class="relative w-full max-w-lg mx-8 md:flex hidden">
			...
			{#if showSuggestions}
				<ul
					class="absolute z-10 w-full bg-white border border-gray-300 rounded mt-12 flex flex-col"
				>
					{#each suggestions as suggestion, index}
						<li class="flex justify-between items-center py-2 border-b border-gray-300">
							<button
								class="flex-1 text-left p-2 hover:bg-gray-200 focus:bg-gray-300 rounded-md {index ===
								selectedIndex
									? 'bg-gray-200'
									: ''}"
		...
		<div class="relative w-full max-w-lg mx-4 md:hidden flex">
			...
			{#if showSuggestions}
				<ul
					class="absolute z-50 left-0 w-full bg-white border border-gray-300 rounded mt-2 shadow-lg"
				>
					{#each suggestions as suggestion, index}
						<li class="flex justify-between items-center py-2 border-b border-gray-300">
							<button
								class="flex-1 text-left p-2 text-sm hover:bg-gray-200 focus:bg-gray-300 rounded-md {index ===
								selectedIndex
									? 'bg-gray-200'
									: ''}"
								on:click={() => selectSuggestion(suggestion)}
