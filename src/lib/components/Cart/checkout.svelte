<script>
    ...
    const calculateTotals = () => {
        subtotal = cartItems.reduce(
            (sum, item) =>  
                sum +
                parseFloat(item.priceSize.price) * parseInt(item.quantity),
            0,
        );
        tax = subtotal * 0;
        total = subtotal + tax;
...
<div class="min-h-screen mx-auto w-full md:w-11/12 p-4 sm:p-6">
    <!-- Main Cart Section -->
    <div class=" flex flex-col xl:flex-row justify-between gap-6">
        {#if loading}
            ...
            >
                <p class=" font-medium text-lg md:text-xl xl:text-2xl">
                    Loading...
                </p>
            </div>
        ...
        {:else}
            <div class="w-full bg-white p-4 rounded-lg shadow-md h-fit mx-auto">
                <h2 class="text-xl mb-3 font-bold">
                    Cart Items <span class="text-red-500"
                ...
                <ul class="">
                    {#each cartItems as item}
                        <li
                            class="flex flex-col lg:flex-row lg:items-center py-3 lg:py-5 border-b-1"
                        >
                            <!-- Product Image and Details -->
                            <h3 class=" lg:hidden mt-3 font-medium text-sm">
                                Product
                            </h3>
                            ...
                            <p
                                class="text-md w-full md:w-2/12 font-semibold text-content"
                            >
                            ...
                            <div class="flex items-center w-60 md:w-2/12">
                                <p
                                    class="w-fit mx-3 text-md font-medium outline-none text-center"
                                >
                                    {item.quantity}
                                </p>
                            </div>
                            ...
                            <div
                                class=" w-full lg:w-2/12 flex justify-between items-center"
                            >
                                <p class="text-md font-semibold">
                                    ₹{(
                                        parseFloat(item.priceSize.price) *
                                        item.quantity
                                    ).toFixed(2)}
                                </p>
                            </div>
    ...
    <div class=" p-4 gap-3 flex flex-col md:flex-row">
        <div class=" w-full md:w-6/12">
            <label for="billing-address" class="block font-bold text-xl my-2"
                >Billing Address</label
            >
            <textarea
                id="billing-address"
                class="w-full mb-4 p-2 h-20 rounded"
                placeholder="Enter your billing address here..."
                bind:value={$billingAddress}
            />
        </div>
        <div class="w-full md:w-6/12">
            <label for="shipping-address" class="block font-bold text-xl my-2"
                >Shipping Address</label
            >
            <textarea
                id="shipping-address"
                class="w-full p-2 h-20 rounded"
                placeholder="Enter your shipping address here..."
                bind:value={$shippingAddress}
            />
        </div>
    </div>
    <div class=" w-10/12 md:w-8/12 mt-2 mx-auto p-4 rounded-lg shadow-md">
        <div>
