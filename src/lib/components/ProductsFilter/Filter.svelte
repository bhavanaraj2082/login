<script>
	...
	function loadCategoriesAndManufacturers($allProducts) {
		...
		$allProducts.forEach((product) => {
			...
			// 	categorySet.add(product.subsubCategory.name);
			// }
	...
	onMount(() => {
		...
		updateFilteredProducts();
		loadFiltersFromUrl();
		loadCategoriesAndManufacturers($allProducts);
	});
	...
	function loadFiltersFromUrl() {
		...
		const categoriesFromUrl = urlParams.get('categories')
			? new Set(urlParams.get('categories').split(','))
			: new Set();
		const manufacturersFromUrl = urlParams.get('manufacturers')
			? new Set(urlParams.get('manufacturers').split(','))
			: new Set();
...
<div class=" px-3 mx-auto flex flex-col lg:flex-row">
	...
	<div class="w-full lg:hidden p-2 mb-4 md:mb-0">
		<div class="border flex flex-col border-gray-300 rounded-lg md:p-4 w-full">
			...
			<h1
				class="text-lg mt-4 font-bold mb-2 text-gray-700 flex items-center justify-center"
				...
				<div class=" filter-group border ml-2 mr-2 border-gray-300 rounded mb-3">
					...
					<div class={showCategoryDropdown ? 'space-y-2' : 'hidden'}>
						...
						<div class="max-h-40 pb-2 overflow-y-auto">
							{#each $filteredCategories.filter((category) => category?.name
									.toLowerCase()
									.includes(localCategorySearchQuery.toLowerCase())) as category}
								<div class="mb-3">
						...
						{#each manufacturers as manufacturer}
							<label class="flex items-center mb-3 text-gray-600 ml-3 text-sm">
								<input
									...
									checked={$selectedManufacturers.has(manufacturer.name)}
								/>
				...
				<div class="  filter-group border ml-2 mr-2 border-gray-300 rounded mb-3">
					...
					<div class={showSortByDropdown ? 'ml-3 space-y-2' : 'hidden'}>
						<div class="flex items-center">
							...
							/>
							<label for="sortByProductName" class="text-gray-600 text-sm"> Product Name </label>
						</div>
						<div class="flex items-center">
							<input
								type="checkbox"
							...
							/>
							<label for="sortByPrice" class="text-gray-600 text-sm"> Price </label>
						</div>
					</div>
				</div>
	...
	<Maincontent on:pageChange={(data) => dispatch('pageChange', data.detail)} />
</div>
