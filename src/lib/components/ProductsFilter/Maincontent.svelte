<script>
	...
	let priceRanges = productsList.map((product) => {
	...
	function updateFilteredProducts() {
		...
		if ($selectedManufacturers.size > 0) {
			filtered = filtered.filter((product) =>
				Array.from($selectedManufacturers).some(
	...
	let loadNextPage = async (currentPage) => {
		...
		const endIndex = Math.min(startIndex + ITEMS_PER_PAGE, productsList.length);
	...
	productsList.priceSize = Array.isArray(productsList.priceSize) ? productsList.priceSize : [];

	onMount(() => {
		const storedFavorites = localStorage.getItem('isFavorited');
		isFavorited = storedFavorites
			? JSON.parse(storedFavorites)
			: new Array(productsList.length).fill(false);

		productsList = productsList.map((product) => {
			const price =
				Array.isArray(product.priceSize) && product.priceSize.length > 0
	...
	export function addToCart(product, quantity) {
		...
		cartState.update((cart) => {
			const existingProductIndex = cart.findIndex(
				(item) => item.partNumber === cartProduct.partNumber
			);
					...
					<p class="text-md font-bold">
						<a
							href={typeof window !== 'undefined'
								? `${window.location.pathname}/${encodeURIComponent(product.productNumber)}`
								: '#'}
						...
						<div class="flex-grow lg:ml-5 ml-3">
							...
							<p>
								<span class="text-gray-600 mt-1"
									>Manufacturer:
									<span class="font-semibold text-gray-600"> {product.manufacturer.name}</span>
								</span>
							</p>
							<p>
								<a href={`/products/${product.category.urlName}`} class="mt-1"
									>Category:
									<span class="font-semibold text-gray-600"> {product.category.name}</span>
								</a>
							</p>
							...
							<div class="flex flex-col md:flex-row items-start justify-between mt-2">
								<div class="flex items-center mb-2 md:mb-0">
									...
									>
									<div
										class="flex flex-col lg:ml-16 md:ml-16 md:flex-row lg:flex-row items-start justify-between"
									>
												...
												{#if product.priceSize && product.priceSize.length > 0}
													{#if product.priceSize.length === 1}
														...
														{:else}
															{product.priceSize[0].price}
														{/if}
													{:else if priceRanges[index] && isPriceInUsd(product.priceSize[0].price)}
														₹{convertUsdToInr(priceRanges[index].min)} - ₹{convertUsdToInr(
											...
											<div class="flex items-center px-3 w-full lg:ml-2 md:ml-5">
												<div class="border text-primary-400 flex items-center rounded">
													...
													<input
														...
														bind:value={productsList[index].quantity}
														readonly
													/>
												...
												<button
													on:click={() => addToCart(product, productsList[index].quantity)}
													class="text-primary-400 hover:bg-primary-500 hover:text-white border border-primary-400 px-2 py-1 ml-3 rounded text-md lg:ml-4"
													>Add To Cart</button
												>
											</div>
										</div>
               ...
               <div class="fixed inset-0 flex items-center justify-center bg-slate-500 bg-opacity-70">
                   <div class="bg-white rounded-lg p-6 w-11/12 max-w-sm md:max-w-lg lg:max-w-xl relative">
                       ...
                       <h2 class="text-xl font-bold mb-1 text-center">
                           {#if isFavorited[productsList.indexOf(modalProduct)]}
			...
			{#if selectedProduct}
				<p class="text-center">
					...
					<select
						...
						class="border w-1/4 p-2 inline-block rounded bg-gray-100 ml-2 hover:border-primary-500 focus:border-primary-400 focus:outline-none focus:ring-0"
					>
