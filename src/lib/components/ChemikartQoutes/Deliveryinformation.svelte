<script>
  ...
  $: iscomplete= $Delivery.Address1 && $Delivery.Address2 && $Delivery.City && $Delivery.Country && $Delivery.Post
  const sub=()=>{
	const pincodePattern = /^\d{6}$/;
		if (!pincodePattern.test($Delivery.Post)) {
			pincodeError = 'Please enter a valid  Post code (6 digits).';
			return;
		}
		else{
		pincodeError = '';
