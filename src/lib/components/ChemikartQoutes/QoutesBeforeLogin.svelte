...
<div class="w-full lg:w-11/12 mx-auto max-w-7xl mb-10">
	<div class="grid grid-cols-1 md:grid-cols-2 gap-12 items-center mb-12 mx-2">
		<div class="space-y-6 sm:p-4 p-2">
			<h1 class="text-2xl font-bold text-gray-800">My Quote</h1>
			<p class="text-gray-600 text-sm md:text-sm max-w-lg">
	...
	<div class="text-center mb-12 mx-2">
		<h2 class="text-2xl font-bold text-gray-800 mb-6">Advanced Features</h2>
		<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
			{#each [{ icon: 'ph:test-tube-fill', title: 'Select custom solution type', desc: 'Customers can choose the specific type of solution that fits their needs. By this customers can tailor their choices based on their unique requirements.' }, { icon: 'wpf:edit-file', title: 'Select custom format', desc: 'You can actively choose your preferences between a pure substance or a solvent substance. ' }, { icon: 'hugeicons:test-tube', title: 'Configure custom solution', desc: 'Easily upload file format (XLSX) , download example XLSX and create your quote.' }, { icon: 'material-symbols:add-notes-outline-rounded', title: 'Additional notes', desc: 'You can describe your preferences with more detail under additional notes.' }, { icon: 'ph:user-list-bold', title: 'Customer details', desc: 'A customer can fill in their details, which helps personalization and ensure smooth communication.' }, { icon: 'material-symbols:delivery-truck-speed-outline-rounded', title: 'Delivery information', desc: 'Customers can provide their delivery details to ensure accurate shipping, timely updates, and a seamless order fulfillment process..' }] as feature}
				<div
					...
					<h3 class="md:text-lg text-md font-semibold text-gray-800">{feature.title}</h3>
	...
	<div class="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center my-3 mx-2">
		...
		<div class="space-y-6">
			...
			<ul class="md:list-disc list-inside text-gray-700 md:text-sm text-xs space-y-2">
...
{#if logInPopup}
	...
	<div class="bg-white p-8 md:p-10 rounded-md shadow-md mx-4 max-w-md relative">
		<h2 class="text-xl md:text-2xl font-semibold mb-4 text-gray-800">Create a Quote</h2>
		<p class="text-sm md:text-sm text-gray-600 mb-6">
		  To create a quote, you need to be registered.
		  Please
		  <a href="/signin" class="text-primary-600 hover:underline underline-offset-2">Sign In</a>
		  or
		  <a href="/signup" class="text-primary-600 hover:underline underline-offset-2">Sign Up</a>.
		</p>
	  
		<button
	      on:click={() => (logInPopup = false)}
		  class="absolute bottom-4 right-4 bg-gray-200 text-gray-700 px-4 py-2 rounded hover:bg-gray-300 transition"
		>
		  Close
		</button>
	  </div>
