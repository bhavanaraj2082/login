...
// 	}
  ...
  {#each data.records as product}
	<div
	  ...
	  {#if !((product?.variants && product?.variants.length > 0) || product?.priceSize?.length === 0)}
		<div
		  class="p-3 space-x-4 justify-between items-center flex-col lg:flex-row m-3 lg:w-1/4 md:w-3/6"
		>
				...
				{#if showPopup}
				  ...
				  >
					...
					>
					  ...
					  >
					  <Icon icon="mdi:close" class="text-xl font-bold" />
					  </button>
				...
				{#if showSharePopup}
				  ...
				  >
					...
					>
					  ...
					  >
					  <Icon icon="mdi:close" class="text-xl text-primary-400 font-bold" />
					  </button>
