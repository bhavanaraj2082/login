...
<section>
  <div class="bg-white">
    <div
      class="w-11/12 max-w-7xl mx-auto p-0 pt-5 border-gray-200 sm:p-4 sm:px-8"
    >
      <h1
        class="sm:text-2xl text-xl font-bold text-gray-700 mb-6 sm:mb-4 md:mb-8 lg:mb-12 ml-4 sm:ml-2 md:ml-2 lg:ml-0"
      >
        ...
        {#each sections as { title }, index}
          ...
          >
            ...
            {#if activeIndex === index}
              <hr
                class="w-24 h-1 bg-primary-500 mt-1 transition-all duration-300 ease-in-out"
              />
            {/if}
          </button>
        {/each}
