<script>
  ...
  const formatTime = (seconds) => {
	...
	$: isButtonDisabled = !email || !password || validError;
</script>

<div
  class="flex flex-col md:flex-row max-w-3xl shadow-lg rounded-lg mx-auto h-auto md:h-auto my-14 border border-gray-300"
>
  ...
  <div
    class="flex flex-col items-center w-full md:w-1/2 rounded-l md:rounded-l-lg p-8 bg-white"
  >
    <div class="w-full mx-auto">
      ...
      >
        ...
        <input
          ...
          class="border border-gray-300 rounded-md p-2 w-full focus:border-primary-400 focus:ring-1 focus:ring-primary-400 placeholder-gray-400 placeholder:text-sm text-sm {errors.email ? 'mb-0' : 'mb-4'} "
        />
        {#if errors.email}
        ...
        <button
          type="submit"
          class="bg-primary-400 hover:bg-primary-500 text-white rounded p-2 w-full text-center"
        >
          Send OTP
        </button>{/if}
                ...
                return async ({ result, update }) => {
                    if (result.type === 'failure') {
                        ...
                        if (redirectTo) {
                            ...
                            toast.error('Verification successful, but no redirect URL provided!');
                        }
                    }
                    await applyAction(result);
                };
        ...
        >
            <div class="mb-4 flex flex-col items-center">
                <p class="text-gray-700 sm:text-sm text-xs mb-3 text-left">
                    Enter the OTP sent to your registered email to complete verification.
                </p>
        ...
        >
            {#if timeLeft > 0}
                <div class="flex justify-center">
                    ...
                    >
                        <span class="text-lg">⏳</span> Time remaining {formatTime(timeLeft)}
                    </span>
                </div>
            {:else}
    ...
    {/if}
      ...
      <button
          ...
          class="w-full flex mb-6 items-center justify-center py-2 px-4 text-white bg-primary-400  rounded-md transition duration-200"
      >
