...
<div class="p-4 flex flex-col lg:flex-row">
	...
	<div class="w-full lg:hidden mb-4 md:mb-0">
		<div
			class=" border md:ml-10 md: mb-5 flex flex-col border-gray-300 rounded-lg shadow-md p-4 w-full"
		>
			<!-- svelte-ignore a11y-click-events-have-key-events -->
			<!-- svelte-ignore a11y-no-noninteractive-element-interactions -->
			<h3
				class="text-lg font-bold mb-2 text-gray-700 flex items-center"
				...
				<div class="filter-group border border-gray-300 rounded mb-3">
					...
					{#if showSearchDropdown}
						<input
							type="text"
							placeholder="Search..."
							class="border hover:border-primary-500 mb-2 rounded p-2 items-center ml-4 w-10/12 text-sm"
			...
			<div class="filter-group border border-gray-300 rounded mb-3">
				...
				{#if showSearchDropdown}
					<input
						type="text"
						placeholder="Search..."
						class="border hover:border-primary-500 rounded p-2 items-center ml-4 mb-2 w-10/12 text-sm"
						on:input={handleSearchChange}
					/>
				{/if}
			</div>
	...
	<div class="w-full md:ml-10 md:w-full lg:w-full md:mr-4 flex flex-col">
		{#if loading}
			<div class="flex items-center justify-center h-full">
				<div
					class="border-4 border-gray-300 border-t-primary-400 rounded-full w-10 h-10 animate-spin"
				></div>
		...
		{:else}
					...
					<div class="border rounded-lg p-4 shadow-lg w-full flex flex-col relative">
								...
								<div class="flex flex-col md:flex-row items-start justify-between mt-2">
									<div class="flex items-center mb-2 md:mb-0">
										...
										<button
											...
											aria-label="View Certificate of Analysis for {product.productName}"
											>CoA</button
										>
												...
												<div class="flex items-center w-full lg:ml-4 md:ml-4">
													<div class="border text-primary-400 flex items-center py-1 rounded">
														<button
															...
															class="font-bold w-8 h-8 bg-gray-100 rounded hover:bg-primary-500 hover:text-white"
															>-</button
														>
													...
													<button
														...
														class="text-primary-400 hover:bg-primary-700 hover:text-white border border-primary-400 p-2 ml-4 rounded text-sm lg:ml-10"
														>Add To Cart</button
													>
												</div>
											</div>
						...
						<div class="flex flex-col items-center justify-between md:hidden mt-2 mx-5">
							<div class="flex justify-between items-center mb-2 w-full">
								...
								<p class="text-gray-600 ml-1">{product.priceSize[0].price}</p>
								<div class="flex items-center justify-between w-full mb-2">
									<div class="border text-primary-400 flex items-center ml-16">
										<button
											on:click={() => decrementQuantity(index)}
											class="font-bold w-8 h-8 bg-gray-100 rounded hover:bg-primary-500 hover:text-white"
											>-</button
										...
										<button
											on:click={() => incrementQuantity(index)}
											class="font-bold w-8 h-8 rounded bg-gray-100 hover:bg-primary-500 hover:text-white"
											>+</button
										>
									</div>
								</div>
							...
							<button
								on:click={() => addToCart(product)}
								class="text-primary-400 hover:bg-primary-700 hover:text-white border border-primary-400 w-40 p-2 rounded text-sm ml-4"
								>Add To Cart</button
							>
						</div>
					</div>
			...
			<div class="flex justify-center mt-4 space-x-4">
				...
				<span class="flex items-center text-primary-400 text-lg"> {currentPage}</span>
				<button
			...
			<div class="fixed inset-0 flex items-center justify-center bg-slate-500 bg-opacity-70">
				<div class="bg-white rounded-lg p-6 w-11/12 max-w-sm md:max-w-lg lg:max-w-xl relative">
					...
					<div class="flex justify-between">
						<button
							on:click={closeModal}
							class="bg-primary-400 text-white p-1 mt-2 rounded w-1/3 mr-2 transition-shadow duration-300 hover:shadow-lg hover:shadow-primary-400"
						>
							Continue Shopping
						</button>
						<button
							on:click={viewFavourites}
							class="bg-primary-400 text-white p-1 rounded w-1/3 transition-shadow duration-300 hover:shadow-lg hover:shadow-primary-400"
						>
							View Favourites
						</button>
					</div>
			...
			<div class="fixed inset-0 flex items-center justify-center bg-neutral-50 bg-opacity-20">
				<div class="bg-white rounded-lg p-6 w-11/12 max-w-sm md:max-w-lg lg:max-w-xl relative">
					...
					<button
						...
						class="bg-primary-400 text-white p-2 rounded w-full mt-4 mb-4">Login</button
					>

					<div class="flex justify-between">
						<a href="/" class="text-primary-400" on:click|preventDefault={closeModal}
							>Continue browsing</a
						>
			...
			<div class="bg-white rounded-lg p-6 w-11/12 md:w-5/12 lg:w-1/2 relative">
				...
				{#if selectedProduct}
					<p class="text-center">
						...
						<select
							bind:value={selectedPackSize}
							class="border p-2 inline-block rounded bg-gray-100 ml-2"
						>
							<!-- Placeholder option -->
					...
					<div class="flex items-center justify-center mt-5">
						...
						<span
							on:click={() => downloadPDFSDS(selectedProduct.safetyDatasheet)}
			...
			<div class="bg-white rounded-lg p-6 w-11/12 md:w-5/12 lg:w-1/2 relative">
				...
				{#if selectedProduct}
					...
					<div class="flex items-center justify-center mt-5">
						...
						<!-- svelte-ignore a11y-no-static-element-interactions -->
						<span
							on:click={() => downloadPDFCOA(selectedProduct.certicateofAnalysis)}
							class="cursor-pointer"
						>
							<Icon icon="mynaui:download" class="text-primary-400 w-8 h-8 text-2xl" />
						</span>
