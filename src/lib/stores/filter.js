...
export let pageNumbersToShow = writable([]);
...
export const pageNumbers = derived([totalPages, currentPage], ([$totalPages, $currentPage]) => {
...
export const updateFilteredProducts = () => {
	...
	filteredProductsList.sort((a, b) => {
		...
		if (sortByValue === 'price') {
			const priceA =
				a.priceSize && a.priceSize.length > 0 ? formatPriceToNumber(a.priceSize[0]?.price) : 0;

			const priceB =
				b.priceSize && b.priceSize.length > 0 ? formatPriceToNumber(b.priceSize[0]?.price) : 0;
...
export const filteredCategories = derived(
	...
	([$categories, $searchQuery]) => {
		//console.log('----$categories---', $categories, $searchQuery);
...
export const updateCategories = (products) => {
	...
	products.forEach((product) => {
		...
		if (product.categories && Array.isArray(product.categories)) {
			product.categories.forEach((category) => categoriesData.push(category));
		} else if (product.category) {
			if (!uniqueSet.includes(product.category.name)) {
				uniqueSet.push(product.category.name);
